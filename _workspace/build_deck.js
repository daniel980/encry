...
function legend3(s, x, y) {
  const items = [[DSX, DSX_T, "DSX OS 기본 제공", 130], [NAVY, NAVY_T, "SKT 개발", 90], [AMB, AMB_T, "SKT 개발 (DSX OS 활용)", 170]];
  let cx = x;
  items.forEach(([c, t, lab, w]) => {
    box(s, cx, y + 2, 11, 11, { fill: t, lineColor: c, lineW: 1 });
    txt(s, lab, cx + 15, y, w, 16, { size: 10.5, color: GRAY_D });
    cx += w + 28;
  });
}
...
/* ============ 슬라이드 2: Executive Summary ============ */
{
  ...
  gov(s, "DSX OS가 제공하는 기반 위에 SKT가 Vera Rubin AI Factory의 E2E 관제를 개발한다 — 오늘 이 구조와 범위 3가지를 확정하고자 한다");
  ...
  box(s, 486, 242, 230, 82, { fill: NAVY_T, lineColor: NAVY, lineW: 2 });
  txt(s, "Vera Rubin AI Factory\nE2E 통합 관제·분석", 492, 250, 218, 44, { size: 12, bold: true, color: NAVY, align: "center", valign: "middle" });
  s.addText([{ text: "목표 수준: ", options: { fontFace: F, fontSize: 10, color: NAVY } }, { text: "CoreWeave 유사", options: { fontFace: F, fontSize: 10, bold: true, color: NAVY } }],
    { x: IN(492), y: IN(296), w: IN(218), h: IN(18), align: "center", margin: 0 });
  chipRect(s, 486, 236, 60, 13, "SKT 개발", { fill: NAVY, color: "FFFFFF", size: 8.5, bold: true });
  txt(s, "DSX OS 기반 위에 SKT가 개발 — AI Factory 통합 관제 (상세: 슬라이드 6)", 730, 250, 176, 70, { size: 10, color: GRAY, valign: "middle" });
  // 행 3 결정 카드
  decisionCard(s, 210, 368, 228, 84, "D1. 아키텍처 방향", "DSX OS 골격 E2E 레이어드 구조 채택");
  decisionCard(s, 446, 368, 228, 84, "D2. 개발·연동 범위", "DSX OS 제공 기반 / SKT 개발 범위 경계 확정");
...
/* ============ 슬라이드 5: AI Factory 전환 ============ */
{
  ...
  const t2 = [
    ...
    [chipCell("대화형 AI 에이전트 (운영 질의응답)", { size: 10 }), sym("●", "Slack에서 헬스·인시던트 질의"), sym("✕", "미보유"), sym("●", "SKT 개발 — DSX MCP 서버 활용")],
  ];
...
/* ============ 슬라이드 6: 목표 아키텍처 (D1) — DSX OS 도메인 기반 + SKT 개발 레이어 ============ */
{
  const s = pres.addSlide();
  header(s, "2-2. 목표 아키텍처 (안)", { badge: "D1" });
  gov(s, "DSX OS가 Compute·Network·Storage·Facility 도메인별 기반 기술을 제공하고, SKT는 그 위에 E2E 관제·분석 레이어를 개발한다", { size: 15.5 });
  ...
  // ── 상단: SKT 개발 레이어 ──
  box(s, 40, 112, 880, 172, { fill: "FFFFFF", lineColor: NAVY, lineW: 1.5 });
  box(s, 40, 112, 880, 24, { fill: NAVY });
  txt(s, "SKT 개발 레이어 — E2E Observability (개발 주체: SKT)", 52, 112, 700, 24, { size: 11.5, bold: true, color: "FFFFFF", valign: "middle" });
  node(56, 144, 420, 34, "E2E 통합 대시보드 (Multi-DC 통합 관제)", "SKT", 10.5);
  node(490, 144, 414, 34, "운영 AI 에이전트 — DSX MCP 서버 카탈로그 활용", "JOINT", 10.5);
  node(56, 186, 206, 40, "이상 탐지·성능 저하 분석", "SKT", 9.5);
  node(270, 186, 152, 40, "장애 예측\n(ECC/XID 선행 신호)", "SKT", 9);
  node(430, 186, 240, 40, "크로스 도메인 상관분석\n— DSX 도메인 데이터 활용", "JOINT", 9);
  node(678, 186, 226, 40, "Facility-aware 상관\n— DSX Exchange 데이터 활용", "JOINT", 9);
  node(56, 234, 206, 40, "시계열 DB·저장소", "SKT", 9.5);
  node(270, 234, 400, 40, "OTel 수집·통합 파이프라인 — DSX 도메인 소스 연동", "JOINT", 9.5);
  node(678, 234, 226, 40, "OOB 수집기 (BMC+Redfish)\n— 전 도메인 횡단, SKT 자체", "SKT", 9),
  // ── 중간: 표준 인터페이스 스트립 ──
  box(s, 40, 296, 880, 22, { fill: GRAY_T, lineColor: LINE });
  txt(s, "표준 인터페이스:  OpenTelemetry (CNCF)  ·  Redfish (DMTF)  ·  MQTT  ·  MCP", 40, 296, 880, 22, { size: 10, bold: true, color: GRAY_D, align: "center", valign: "middle" });
  [150, 390, 630, 830].forEach((x) => arrow(s, x, 296, x, 284, {}));
  [150, 390, 630, 830].forEach((x) => arrow(s, x, 330, x, 318, {}));
  // ── 하단: DSX OS Reference 기반 (도메인별) ──
  box(s, 40, 330, 880, 138, { fill: "FFFFFF", lineColor: DSX, lineW: 1.5 });
  box(s, 40, 330, 880, 24, { fill: DSX });
  txt(s, "NVIDIA DSX OS Reference 기반 — 도메인별 제공 기술 (제공 주체: DSX OS·Vera Rubin)", 52, 330, 760, 24, { size: 11.5, bold: true, color: "FFFFFF", valign: "middle" });
  const doms = [
    ["Compute", "DCGM 텔레메트리 (GPU In-band)", "Health Automation · Resiliency"],
    ["Network", "UFM · Spectrum-6 패브릭 텔레메트리", "NMX Manager"],
    ["Storage", "BlueField-4 STX · NVMe-oF 텔레메트리", "DPU 텔레메트리 오프로드"],
    ["Facility", "DSX Exchange (MQTT IT/OT 허브)", "전력·열·그리드 이벤트 노출"],
  ];
  doms.forEach(([dn, a, b], i) => {
    const x = 56 + i * 216;
    box(s, x, 360, 200, 18, { fill: "FFFFFF", lineColor: DSX });
    txt(s, dn, x, 360, 200, 18, { size: 10, bold: true, color: "3E6300", align: "center", valign: "middle" });
    node(x, 382, 200, 28, a, "DSX", 8.5);
    node(x, 414, 200, 28, b, "DSX", 8.5);
  });
  box(s, 56, 446, 848, 18, { fill: DSX_T, lineColor: DSX });
  txt(s, "공통 기반: MCP 서버 카탈로그 · Lifecycle · Runtime Consistency · Multi-tenant · AI Platform Services (6개 모듈 — 상세: 백업 B, 15p)", 56, 446, 848, 18, { size: 9, color: "3E6300", align: "center", valign: "middle" });
  legend3(s, 40, 474);
  txt(s, "DSX OS: 2026-05 GTC Taipei 발표, 오픈소스·모듈러", 640, 474, 280, 16, { size: 9.5, color: GRAY, align: "right" });
  pageNo(s, 6);
...
/* ============ 슬라이드 7: 범위 매트릭스 (D2) ============ */
{
  ...
  function mc(kind, label, body, o = {}) { // matrix cell
    const map = { DSX: [DSX_T, "3E6300", "DSX"], SKT: [NAVY_T, NAVY, "SKT"], JOINT: [AMB_T, "8A5000", "SKT+DSX"], NONE: [GRAY_T, GRAY, ""] };
  ...
  const t7 = [
    ...
    [chipCell("Compute (GPU·노드)", { size: 10 }), mc("DSX", 0, "DCGM·Health Automation 통합"), mc("JOINT", 0, "OTel 파이프라인·시계열 DB (DSX 소스 연동)"), mc("SKT", 0, "이상 탐지·장애 예측·잡 상관", { star: true }), mc("SKT", 0, "통합 대시보드"), mc("DSX", 0, "Resiliency 모듈 (정책은 SKT 정의)", { star: true })],
    [chipCell("Network (패브릭)", { size: 10 }), mc("DSX", 0, "UFM·Spectrum 텔레메트리"), mc("NONE", 0, "(공통 파이프라인 공용)"), mc("JOINT", 0, "패브릭-잡 상관분석 — UFM 데이터 활용"), mc("SKT", 0, "통합 대시보드"), mc("DSX", 0, "Resiliency")],
    [chipCell("Storage", { size: 10 }), mc("DSX", 0, "STX·NVMe-oF 텔레메트리 노출"), mc("NONE", 0, "(공통 파이프라인 공용)"), mc("SKT", 0, "I/O 병목·용량 분석"), mc("SKT", 0, "통합 대시보드"), mc("DSX", 0, "Resiliency", { star: true })],
    [chipCell("Facility (전력·냉각)", { size: 10 }), mc("DSX", 0, "DSX Exchange (MQTT)"), mc("NONE", 0, "(공통 파이프라인 공용)"), mc("JOINT", 0, "Facility-aware 상관 — Exchange 데이터 활용 (SKT 경험)"), mc("SKT", 0, "통합 대시보드"), mc("JOINT", 0, "SKT 운영 절차 주관 — Resiliency 연계", { star: true })],
  ...
  s.addText([
    { text: "주체 지정 20셀 중 SKT 개발 13 (자체 9 + DSX 활용 4) · DSX OS 기본 제공 7 · 논의 필요 ★4", options: { bold: true, fontSize: 10 } },
    { text: " → 개발 주체는 SKT — 기반 7개 영역은 DSX OS가 제공해 개발 부담 경감", options: { fontSize: 10 } },
  ].map(r => ({ text: r.text, options: Object.assign({ fontFace: F, color: T900 }, r.options) })), { x: IN(634), y: IN(416), w: IN(274), h: IN(48), margin: 0 });
  legend3(s, 40, 476);
  txt(s, "총 25셀 = 주체 지정 20 + 공용 4 + 해당 없음 1 (★는 오버레이) · SKT+DSX = DSX OS 활용해 SKT가 개발", 620, 476, 300, 14, { size: 9, color: GRAY });
  pageNo(s, 7);
...
/* ============ 슬라이드 9: Compute 개발 방안 ============ */
{
  ...
  s.addText([
    ...
    { text: " SKT+DSX ", options: { color: "FFFFFF", bold: true, highlight: AMB } }, { text: " 잡 메타 연계 커넥터 (스케줄러 연동)    ", options: {} },
...
/* ============ 슬라이드 11: Network·Storage·Facility (D3) ============ */
{
  ...
  const cards = [
    ["Network", "1", DSX, DSX_T, [
      ...
      ["개발 항목", "패브릭-잡 상관분석 (잡 성능 저하 ↔ 네트워크 혼잡 매핑) — SKT, UFM 데이터 활용"],
    ...
    ["Facility", "3", GRAY, GRAY_T, [
      ...
      ["개발 항목", "Facility-aware 상관분석 (SKT 기존 경험 접목 [확인: SKT]), 전력-워크로드 연계 — SKT, Exchange 데이터 활용"],
...
/* ============ 슬라이드 12: 결정 요청 및 다음 단계 ============ */
{
  ...
  decisionCard(s, 339, 114, 283, 104, "D2. 개발·연동 범위", "범위 매트릭스 기준 SKT 개발 범위 확정 — ★4개 항목 포함", "근거: 슬라이드 7");
...
/* ============ 백업 C ============ */
{
  ...
  const tC = [
    ...
    [chipCell("대화형 AI 에이전트", { size: 10 }), cell("Slack에서 클러스터 헬스·잡·인시던트·변경사항 질의응답", { size: 10.5 }), cell("SKT 개발 — DSX OS MCP 서버 카탈로그 활용 (Phase 3)", { size: 10.5, fill: AMB_T })],
  ];
