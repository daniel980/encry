---
name: presentation-dataviz-guide
description: "데이터 시각화 차트 선택 가이드, 정보 계층 설계 원칙, 숫자-차트 매핑 매트릭스를 제공하는 presentation-architect 확장 스킬. '차트 선택', '데이터 시각화', '그래프 종류', '정보 설계', '차트 컬러', '데이터 스토리텔링' 등 데이터를 시각적으로 표현할 때 사용한다. 단, 실제 차트 렌더링이나 BI 도구 연동은 이 스킬의 범위가 아니다."
---

# Data Visualization Guide — 데이터 시각화 차트 선택 & 정보 설계 가이드

presentation-architect 에이전트가 데이터 시각화 설계 시 활용하는 차트 선택 매트릭스, 정보 계층 원칙, 시각화 베스트 프랙티스.

## 대상 에이전트

`presentation-architect` — 이 스킬의 차트 선택 로직과 정보 설계 원칙을 데이터 시각화에 직접 적용한다.
